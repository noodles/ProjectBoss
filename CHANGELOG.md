...
- First-run setup now requires **both** stdin and stdout to be a terminal before it asks anything. stdin alone was not enough: a pty with nobody on the other end is still a tty, so a captured or piped run waited forever for an answer nobody could give. It also means `pb list --json | jq` never prompts
...
- The Homebrew formula is called `projectboss`, not `pb`, so `brew install projectboss` works without naming the tap. `brew install pb` resolves to an unrelated Pushbullet cask and always would have. The command installed is still `pb`
- `brew trust noodles/tap` is a required step. That is Homebrew's policy for every third-party tap, not something this project can avoid; verified by revoking trust and watching the install refuse
- Homebrew pulls in a Python, roughly 80MB, for `pb` to run on. Said plainly in the README rather than left to surprise anyone, since the project otherwise advertises no dependencies
