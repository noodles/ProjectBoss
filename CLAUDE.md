...
ProjectBoss is a local CLI tool (`pb`) for creating, finding, and managing projects with consistent directory structure, a searchable index, and automatic staleness tracking. It is a **single-file Python 3 application** (`pb.py`, ~1400 lines) with **zero external dependencies**: pure stdlib only.

## Running

```bash
# Direct execution
python3 pb.py <command> [options]
...
They live in `test_proj.py` and never touch the real `~/.pb`: disk tests use a
temp directory, and the data-layer tests repoint `pb`'s module-level paths at
it. There is no build step and no linter configured.

## Architecture

`pb.py` is organized in clearly delimited sections:

1. **Constants**: `PB_DIR` (~/.pb/), paths to config/index/ignored JSON files, `DEFAULT_CONFIG`
...
5. **Command handlers**: `cmd_config`, `cmd_new`, `cmd_list`, `cmd_info`, `cmd_edit`, `cmd_open`, `cmd_rescan`, `cmd_delete`, `cmd_ignore`, `cmd_idea`, `cmd_adr`, `cmd_help`
...
- **Flexible queries**: `find_entry()` resolves projects by numeric ID, ID prefix, name substring, or slug, handling ambiguity with user-friendly errors
- **Shell integration**: `pb open` requires a shell function (installed by `install.sh`) to `cd` into project directories
...
1. Bump `VERSION` in `pb.py` (patch version for fixes/features, minor for larger changes)
